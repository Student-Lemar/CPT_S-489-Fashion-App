...
router.post("/extract/:itemId", authenticate, async (req, res) => {
  try {
    ...
    if (!item.imageDataUrl) {
      return res
        .status(422)
        .json({ error: "Item has no image — upload an image first" });
    }
    ...
    if (!detected) {
      return res
        .status(422)
        .json({ error: "Could not extract a color from this image" });
    }
...
router.post("/generate", authenticate, async (req, res) => {
  try {
    ...
    if (useImageColors) {
      const needsExtraction = rawItems.filter(
        (i) => i.imageDataUrl && !i.colorExtracted,
      );
      await Promise.all(
    ...
    const suggestions = generateSuggestions(
      enriched,
      { harmony, occasion, style, preferredColor },
      capped,
    );
    ...
    if (!suggestions.length) {
      return res.status(422).json({
        error:
          "Not enough items to generate outfits — you need at least one top, one bottom, and one pair of shoes",
      });
    }
