...
router.post('/users/:username/toggle-status', async (req, res) => {
  try {
    ...
    await user.save();
    await logAction(req.user.username, newStatus === 'suspended' ? 'SUSPEND_USER' : 'REACTIVATE_USER', req.params.username);
    res.json(user);
...
router.get('/reports', async (req, res) => {
  try {
    ...
    res.json(reports);
  } catch (err) { res.status(500).json({ error: 'Failed to list reports' }); }
});
...
router.put('/reports/:id', async (req, res) => {
  try {
    ...
    await report.save();
    await logAction(req.user.username, `UPDATE_REPORT_${report.status.toUpperCase()}`, report.id);
    res.json(report);
...
router.get('/audit-log', async (req, res) => {
  try {
    const log = await AuditLogEntry.findAll({ order: [['timestamp', 'DESC']], limit: 100 });
    res.json(log);
