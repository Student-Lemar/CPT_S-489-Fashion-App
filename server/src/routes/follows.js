...
async function getStatus(follower, followed) {
  const row = follower ? await Follow.findOne({ where: { followerUsername: follower, followedUsername: followed } }) : null;
...
router.get('/:username/status', authenticate, async (req, res) => {
  try {
    ...
    res.json(status);
  } catch (err) { res.status(500).json({ error: 'Failed to get follow status' }); }
});
...
router.post('/:username', authenticate, async (req, res) => {
  ...
  try {
    await Follow.findOrCreate({
      where: { followerUsername: req.user.username, followedUsername: req.params.username },
    });
    const status = await getStatus(req.user.username, req.params.username);
    res.json(status);
...
router.delete('/:username', authenticate, async (req, res) => {
  try {
    await Follow.destroy({ where: { followerUsername: req.user.username, followedUsername: req.params.username } });
    const status = await getStatus(req.user.username, req.params.username);
    res.json(status);
