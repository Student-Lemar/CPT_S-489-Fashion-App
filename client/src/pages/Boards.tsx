    ...
    <div className="page">
      <div className="container">
        <div className="page-header">
          ...
          <button className="primary-link" onClick={() => navigate('/boards/create')}>
            + New Board
          </button>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="empty-state">
            <p>No boards yet. <button className="text-btn" onClick={() => navigate('/boards/create')}>Create one →</button></p>
          </div>
        ) : (
          <div className="boards-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
            {filtered.map((board) => {
              const boardOutfits = outfitsMap[board.id] ?? [];
              const icons = boardOutfits.flatMap((o) => o.itemIcons).slice(0, 3);
              return (
                <article
                  key={board.id}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <h2 style={{ fontSize: '16px', fontWeight: 700, margin: 0 }}>{board.name}</h2>
                    <span className={`badge ${board.visibility}`}>{board.visibility}</span>
                  </div>
                  <p style={{ fontSize: '13px', color: '#666', marginBottom: '12px' }}>
                    {board.description || 'No description yet.'}
                  </p>
                  <div style={{ fontSize: '1.8rem', marginBottom: '10px' }}>
                    {icons.length ? icons.map((ic, i) => <span key={i}>{ic}</span>) : <>🧥 👖 👟</>}
                  </div>
                  <p style={{ fontSize: '12px', color: '#888' }}>
                    {board.outfitIds.length} outfit{board.outfitIds.length === 1 ? '' : 's'} • {formatDate(board.createdAt)}
                  </p>
                </article>
              );
            })}
