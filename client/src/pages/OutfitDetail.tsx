...
export default function OutfitDetail() {
  ...
  async function handleSave(e: FormEvent) {
    ...
    try {
      await persist(false);
      setStatusOk(true);
      setStatus("Outfit saved successfully.");
    } catch {
      setStatusOk(false);
      setStatus("Failed to save outfit.");
    }
  }
      ...
      if (boards.length) {
        ...
        if (!board.outfitIds.includes(saved.id)) {
          await boardsApi.update(board.id, {
            outfitIds: [...board.outfitIds, saved.id],
          });
        }
        setStatusOk(true);
        setStatus(`Outfit posted to ${board.name}.`);
        setTimeout(
        ...
        setStatusOk(true);
        setStatus("Outfit saved. Create a board to post it.");
        setTimeout(
          () =>
            navigate(`/boards/create?outfitId=${encodeURIComponent(saved.id)}`),
          450,
        );
      }
    } catch {
      setStatusOk(false);
      setStatus("Failed to post outfit.");
    }
  }
      ...
      <div className="container">
        <div className="page-header">
          <div>
            ...
            <p>
              {isDraft
                ? "Review and save your generated outfit."
                : "Edit your outfit."}
            </p>
          </div>
          <button
