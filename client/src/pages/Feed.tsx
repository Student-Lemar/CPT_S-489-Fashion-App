...
export default function Feed() {
  ...
  useEffect(() => {
    feedApi.list().then(setPosts).catch(console.error).finally(() => setLoading(false));
  }, []);
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
            {filtered.map((post) => (
              ...
              >
                <div style={{ background: '#f8f9fa', padding: '20px', textAlign: 'center', fontSize: '2rem' }}>
                  {post.items.map((icon, i) => (
                    <span key={i} style={{ marginRight: '4px' }}>{icon}</span>
                  ))}
                </div>
                <div style={{ padding: '14px' }}>
                  ...
                  <p style={{ fontSize: '13px', color: '#555', margin: 0 }}>{post.caption}</p>
                </div>
              </Link>
            ))}
          </div>
