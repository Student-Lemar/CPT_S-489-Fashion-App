...
export default function AdminModeration() {
  ...
  useEffect(() => {
    adminApi.listReports().then(setReports).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  const filtered = reports.filter((r) => {
    return (!typeFilter || r.type === typeFilter) && (!statusFilter || r.status === statusFilter);
  });
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {filtered.map((report) => (
              <div
                key={report.id}
                className="card"
                style={{ padding: '18px', opacity: report.status !== 'pending' ? 0.65 : 1 }}
              >
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '10px' }}>
                  <span
                    style={{
                      background: report.type === 'post' ? '#fef0f6' : '#f0f9ff',
                  ...
                  <span style={{ fontWeight: 600, fontSize: '14px' }}>{report.contentLabel}</span>
                  <span style={{ fontSize: '12px', color: '#888' }}>Reported {formatDate(report.createdAt)}</span>
                  <span
                    style={{
                      marginLeft: 'auto', fontSize: '11px', fontWeight: 700,
                      color: report.status === 'pending' ? '#92400e' : '#166534',
                      background: report.status === 'pending' ? '#fef3c7' : '#dcfce7',
                ...
                <div style={{ fontSize: '14px', marginBottom: '10px' }}>
                  ...
                  <p style={{ margin: 0, color: '#666', fontStyle: 'italic' }}>"{report.caption}"</p>
                </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    {Object.entries(ACTION_LABELS).map(([action, label]) => (
                      <button
                        ...
                        className="btn"
                        style={{
                          fontSize: '12px', padding: '5px 12px', borderRadius: '8px', border: 'none', cursor: 'pointer',
                          background: action === 'remove' ? '#fee2e2' : action === 'hide' ? '#fef3c7' : action === 'warn' ? '#fff7ed' : '#f0f9ff',
                          color: action === 'remove' ? '#b42318' : action === 'hide' ? '#92400e' : action === 'warn' ? '#c2410c' : '#0369a1',
                          fontWeight: 700,
                        }}
        ...
        {toast && (
          <div
            style={{
              ...
              background: '#1e1e1e', color: '#fff', padding: '10px 20px', borderRadius: '10px',
              fontSize: '14px', fontWeight: 600, zIndex: 1000, boxShadow: '0 4px 16px rgba(0,0,0,0.2)',
            }}
          >
            {toast}
          </div>
