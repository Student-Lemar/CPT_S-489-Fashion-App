...
function buildCombo(items: EnrichedItem[]): OutfitCombo {
  const get = (cat: string) =>
    items.find((i) => i.category === cat) as EnrichedItem;
  const extras = items.filter(
    (i) => i.category === "outerwear" || i.category === "accessories",
  );
  return {
    top: get("tops"),
    bottom: get("bottoms"),
...
function buildReason(
  ...
  const colors = [
    ...new Set(items.map((i) => capitalize(i.aiColor.colorName))),
  ];
...
function buildInsights(
  ...
  return [
    ...
    neutralCount >= 2
      ? "Neutrals keep the look grounded."
      : "The palette uses contrast for visual energy.",
    imageScanned
      ? `Image color scan influenced ${imageScanned} item${imageScanned === 1 ? "" : "s"}.`
      : "Colors sourced from item labels.",
...
export default function OutfitGenerator() {
  ...
  async function generate(randomMode: boolean) {
    ...
    setSelected(null);
    setStatus(
      "Asking the server to analyse your wardrobe colors and build suggestions…",
    );
    setStatusOk(false);
    try {
      const filters = randomMode
        ...
        : {
            harmony:
              harmony === ("auto" as HarmonyMode) ? ("auto" as const) : harmony,
            occasion,
            style,
            preferredColor,
            useImageColors,
      ...
      const result = await colorsApi.generate({
        ...filters,
        maxSuggestions: 3,
      });

      const activeOccasion: OccasionMode =
        (filters.occasion as OccasionMode) ?? "everyday";
      ...
      if (mapped.length) {
        ...
        sessionStorage.setItem(
          "fashion_generated_outfit_v1",
          JSON.stringify(buildDraft(mapped[0])),
        );
        setStatus(
          `Generated ${mapped.length} suggestion${mapped.length === 1 ? "" : "s"}.`,
        );
        setStatusOk(true);
      } else {
        setStatus(
          "Not enough wardrobe items. Add at least a top, bottom, and shoes.",
        );
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="page">
      <div className="container">
        ...
        {/* Controls */}
        <div
          className="card generator-controls"
        ...
        >
          ...
          >
            ...
            <div className="field">
              ...
              <select
                value={occasion}
                onChange={(e) => setOccasion(e.target.value as OccasionMode)}
              >
                {OCCASIONS.map((o) => (
          ...
          >
            ...
            />
            Use image color scan — server will extract colors from item photos
            (slower)
          </label>
        ...
        {status && (
          <p
            className={`status ${statusOk ? "success" : ""}`}
            aria-live="polite"
          ...
          {selected && (
            <div className="card" style={{ padding: "20px" }}>
              ...
              >
                {[
                  selected.combo.top,
                  selected.combo.bottom,
                  selected.combo.shoes,
                ].map((item, i) => (
                ...
                {comboItems(selected.combo).map((item, i) => (
                  <span
                    ...
                    style={{
                      ...
                      color:
                        item.aiColor.colorName === "white" ? "#222" : "#fff",
                      padding: "4px 10px",
                      borderRadius: "20px",
                      fontSize: "12px",
                      fontWeight: 600,
