    ...
    boardsApi
      ...
      .then(async (b) => {
        ...
        const [outfitResults, profile] = await Promise.all([
          Promise.all(
            b.outfitIds.map((oid) => outfitsApi.get(oid).catch(() => null)),
          ),
    ...
    return (
      <div className="page">
        <div className="container">
          <p className="status error">{error || "Board not found."}</p>
        </div>
      </div>
    );
      ...
      <div className="container">
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          >
            <div>
              <span
                className={`badge ${board.visibility}`}
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button
                ...
                onClick={() =>
                  navigate(
                    `/boards/create?edit=${encodeURIComponent(board.id)}`,
                  )
                }
              >
                Edit
          ...
          {ownerProfile && (
            ...
            >
              ...
              >
                {(ownerProfile.displayName || ownerProfile.username)
                  .slice(0, 1)
                  .toUpperCase()}
              </div>
              <div>
        ...
        {posts.length === 0 ? (
          ...
          >
            {posts.map((post) => {
              const icons = post.itemIcons.length
                ? post.itemIcons
                : ["👕", "👖", "👟"];
              return (
                <div
