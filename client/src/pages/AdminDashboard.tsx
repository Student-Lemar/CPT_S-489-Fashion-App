...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([adminApi.listUsers(), adminApi.listReports(), adminApi.auditLog()])
      .then(([u, r, a]) => { setUsers(u); setReports(r); setAuditLog(a); })
      .catch(console.error)
      .finally(() => setLoading(false));
  }, []);

  const pending = reports.filter((r) => r.status === 'pending').length;
  const active = users.filter((u) => u.role === 'creator' && u.status === 'active').length;
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '28px', flexWrap: 'wrap' }}>
          ...
          <Link className="btn btn-secondary" to="/admin/moderation">
            Moderation Queue
            {pending > 0 && (
              <span style={{ background: '#ff4d8d', color: '#fff', borderRadius: '12px', padding: '1px 7px', fontSize: '11px', marginLeft: '6px' }}>
                {pending}
              </span>
            )}
          </Link>
        ...
        <section className="card" style={{ padding: '20px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '14px' }}>Recent Audit Log</h2>
          {auditLog.length === 0 ? (
              ...
              <tbody>
                {auditLog.slice(0, 5).map((entry) => (
                  <tr key={entry.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    ...
                    <td style={{ padding: '8px 12px' }}>{entry.adminUsername}</td>
                    <td style={{ padding: '8px 12px' }}>
                      <span style={{ background: '#f6f2ff', color: '#6440d6', borderRadius: '6px', padding: '2px 8px', fontSize: '11px', fontWeight: 700 }}>
                        {entry.action}
                      </span>
                    </td>
